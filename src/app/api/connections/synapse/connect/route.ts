...
const Body = z.object({
  name: z.string().min(1).max(120).default("Azure Synapse"),
...
export async function POST(req: Request) {
  ...
  try {
    // Default the schema filter to `dbo` (the SQL Server / Synapse
    // default schema) when blank — avoids the sys + INFORMATION_SCHEMA
    // discovery that collides with the BQ-reserved `information_schema`
    // prefix in target-bigquery.
    ...
    const responseBody = connectErrorEnvelope("synapse", step.current, err, [
      body.password,
    ]);
    console.error(
      "[synapse/connect]",
