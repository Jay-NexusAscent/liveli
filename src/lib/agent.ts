...
- **Always alias aggregate / expression columns.** \`COUNT(*)\`, \`SUM(amount)\`, \`AVG(...)\`, \`DATE(created_at)\`, any computed column — assign a meaningful \`AS\` name. Without an alias BigQuery returns the column as \`f0_\`, \`f1_\`, \`f2_\` etc. which is ugly to display. Example: \`SELECT COUNT(*) AS total_sessions FROM ...\` not \`SELECT COUNT(*) FROM ...\`.
