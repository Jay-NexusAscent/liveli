...
interface ZendeskSellWizardProps {
  open: boolean;
  onClose: () => void;
  onConnected: () => void;
}
...
export function ZendeskSellWizard({ open, onClose, onConnected }: ZendeskSellWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/zendesk-sell/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Zendesk Sell" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="Access token"
          hint="Zendesk Sell → Settings → OAuth → Access Tokens. This is the Sell CRM, not Zendesk Support."
        >
          <input
            type="password"
            required
