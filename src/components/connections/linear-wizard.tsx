...
export function LinearWizard({ open, onClose, onConnected }: LinearWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // Linear personal API keys start with "lin_api_". OAuth client
    // tokens start with "lin_oauth_" but we don't support those flows.
    if (!/^lin_api_/.test(key)) {
      setError(
        "Linear Personal API keys start with lin_api_. Create one at Linear → Settings → API → Personal API keys."
      );
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/linear/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Linear" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          ...
          hint="Linear → Settings → API → Personal API keys → New API key. Read access to your workspace's issues, cycles, projects, teams."
        >
          <input
            type="password"
            required
