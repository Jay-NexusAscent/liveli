...
interface SqlserverWizardProps {
  open: boolean;
  onClose: () => void;
  onConnected: () => void;
}
...
export function SqlserverWizard({ open, onClose, onConnected }: SqlserverWizardProps) {
  ...
  // If the user pastes a full `sqlserver://`/`mssql://` URL into Host,
  // split it apart — same support-saving auto-parse the postgres/mysql
  // wizards do.
  const handleHostChange = (raw: string) => {
    const trimmed = raw.trim();
    if (/^(sqlserver|mssql):\/\//i.test(trimmed)) {
      try {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/sqlserver/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect SQL Server" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="grid grid-cols-[1fr_120px] gap-3">
          <Field
            label="Host"
            hint="Paste a full sqlserver:// connection string here and the other fields will auto-fill."
          >
            <input
              type="text"
              required
              ...
              placeholder="db.example.com — or paste a full sqlserver:// URL"
              className={inputClass}
            />
          </Field>
          <Field label="Port">
        ...
        <Field
          ...
          hint="Defaults to dbo. Add others if your tables live in additional schemas you want replicated together."
        >
          <input
            type="text"
            value={form.schemas}
