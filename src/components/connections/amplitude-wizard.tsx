...
interface AmplitudeWizardProps {
  open: boolean;
  onClose: () => void;
  onConnected: () => void;
}
...
export function AmplitudeWizard({ open, onClose, onConnected }: AmplitudeWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!key || !secret) {
      setError(
        "Both API Key and Secret Key are required. Find them in Amplitude → Settings → Projects → [your project] → General."
      );
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/amplitude/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, apiKey: key, apiSecret: secret }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Amplitude" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="API Key"
          hint="Amplitude → Settings → Projects → [your project] → General. Public, identifies the project."
        >
          <input
            type="text"
            required
        ...
        <Field
          label="Secret Key"
          hint="From the same screen as the API Key. Secret — never shared with the client."
        >
          <input
            type="password"
            required
