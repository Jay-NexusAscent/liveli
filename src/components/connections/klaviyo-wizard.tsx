...
export function KlaviyoWizard({ open, onClose, onConnected }: KlaviyoWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // Klaviyo Private API Keys start with "pk_". Public keys (site IDs)
    // are a 6-char alphanumeric — those can't read account data, so
    // reject them early with a useful message.
    if (!/^pk_/i.test(key)) {
      setError(
        "Klaviyo Private API Keys start with pk_. Find one in Klaviyo → Settings → Account → API Keys (Private Keys section)."
      );
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/klaviyo/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, apiKey: key }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Klaviyo" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          ...
          hint="Klaviyo → Settings → Account → API Keys → Create Private API Key. Needs read access to Lists, Profiles, Campaigns, Flows, Events."
        >
          <input
            type="password"
            required
