...
export function SegmentWizard({ open, onClose, onConnected }: SegmentWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/segment/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Segment" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="API token"
          hint="Segment workspace → Settings → Access Management → Tokens. Reads the Config API (sources, destinations)."
        >
          <input
            type="password"
            required
