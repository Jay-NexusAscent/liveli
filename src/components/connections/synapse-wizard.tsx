...
export function SynapseWizard({ open, onClose, onConnected }: SynapseWizardProps) {
  ...
  const handleHostChange = (raw: string) => {
    ...
    if (/^(sqlserver|mssql|synapse):\/\//i.test(trimmed)) {
      try {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/synapse/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Azure Synapse" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        <Field label="Connection name">
          <input
            ...
            placeholder="e.g. Synapse warehouse"
        ...
        <div className="grid grid-cols-[1fr_120px] gap-3">
          <Field
            label="Server"
            hint="Your dedicated SQL pool endpoint, e.g. myws.sql.azuresynapse.net"
          >
            <input
              type="text"
              required
        ...
        <Field
          ...
          hint="Defaults to dbo. Add others if your tables live in additional schemas."
        >
          <input
            type="text"
            value={form.schemas}
            onChange={(e) => update("schemas", e.target.value)}
            placeholder="e.g. dbo, staging"
            className={inputClass}
          />
        </Field>
        ...
        <label className="flex items-center gap-2 text-[13px] text-text-secondary">
          ...
          />
          Use SSL (required by Azure Synapse)
        </label>
