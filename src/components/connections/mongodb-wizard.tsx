...
export function MongodbWizard({ open, onClose, onConnected }: MongodbWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/mongodb/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, connectionString: form.connectionString.trim() }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect MongoDB" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          ...
          hint="Your full mongodb:// or mongodb+srv:// URI, including username and password."
        >
          <input
            type="password"
            required
        ...
        <Field label="Database" hint="The database to replicate.">
          <input
            type="text"
            required
            value={form.database}
