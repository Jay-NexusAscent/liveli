...
export function MixpanelWizard({ open, onClose, onConnected }: MixpanelWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!secret || secret.length < 16) {
      setError(
        "Mixpanel Project API Secret is required. Find it in Mixpanel → Project Settings → Access Keys → Service Account or legacy API Secret."
      );
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/mixpanel/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, apiSecret: secret }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Mixpanel" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          ...
          hint="Mixpanel → Project Settings → Access Keys. Use a Service Account secret or the legacy Project API Secret."
        >
          <input
            type="password"
            required
