...
export function HubspotWizard({ open, onClose, onConnected }: HubspotWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // HubSpot Private App tokens start with "pat-na1-" / "pat-eu1-" etc.
    if (!/^pat-/.test(token)) {
      setError(
        "That doesn't look like a HubSpot Private App access token — it should start with pat-."
      );
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/hubspot/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect HubSpot" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          ...
          hint="Create a Private App in HubSpot Settings → Integrations and grant CRM read scopes. Starts with pat-."
        >
          <input
            type="password"
            required
        ...
        <Field
          ...
          hint="Earliest date to replicate from. Leave blank for everything since 2020."
        >
          <input
            type="date"
