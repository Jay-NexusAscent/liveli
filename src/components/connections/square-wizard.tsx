...
export function SquareWizard({ open, onClose, onConnected }: SquareWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/square/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, accessToken: form.accessToken.trim() }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Square" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="Access token"
          hint="Square Dashboard → Developer → Applications → your app → Production access token (or Sandbox)."
        >
          <input
            type="password"
            required
        ...
        <label className="flex items-center gap-2 text-[13px] text-text-secondary">
          ...
          />
          This is a Sandbox access token (test data)
        </label>
