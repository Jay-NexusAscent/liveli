...
interface PipedriveWizardProps {
  open: boolean;
  onClose: () => void;
  onConnected: () => void;
}
...
export function PipedriveWizard({ open, onClose, onConnected }: PipedriveWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/pipedrive/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, apiToken: form.apiToken.trim() }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Pipedrive" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="API token"
          hint="Pipedrive → Settings → Personal preferences → API. Copy your personal API token."
        >
          <input
            type="password"
            required
