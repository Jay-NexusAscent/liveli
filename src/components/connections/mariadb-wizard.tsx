...
export function MariadbWizard({ open, onClose, onConnected }: MariadbWizardProps) {
  ...
  // MariaDB uses the mysql:// URI scheme. Split a pasted connection
  // string apart, same convenience as the MySQL wizard.
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/mariadb/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect MariaDB" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="grid grid-cols-[1fr_120px] gap-3">
          <Field
            label="Host"
            hint="Paste a full mysql:// or mariadb:// connection string here and the other fields will auto-fill."
          >
            <input
              type="text"
              required
              ...
              placeholder="db.example.com — or paste a full connection URL"
              className={inputClass}
            />
          </Field>
          <Field label="Port">
        ...
        <Field
          ...
          hint="Defaults to the database above. Useful if your MariaDB user can see multiple schemas you want replicated together."
        >
          <input
            type="text"
