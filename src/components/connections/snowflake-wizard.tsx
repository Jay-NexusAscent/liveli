...
export function SnowflakeWizard({ open, onClose, onConnected }: SnowflakeWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/snowflake/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Snowflake" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="Account"
          hint="Your Snowflake account locator, e.g. xy12345.eu-west-1 or the org-account form."
        >
          <input
            type="text"
            required
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Warehouse" hint="Compute warehouse the sync runs against.">
            <input
              type="text"
              required
        ...
        <Field
          ...
          hint="Leave blank to replicate all schemas in the database."
        >
          <input
            type="text"
            value={form.schema}
