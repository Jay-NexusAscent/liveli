...
export function CloseWizard({ open, onClose, onConnected }: CloseWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/close/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, apiKey: form.apiKey.trim() }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Close" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="API key"
          hint="Close → Settings → API Keys. A read-only key is enough."
        >
          <input
            type="password"
            required
