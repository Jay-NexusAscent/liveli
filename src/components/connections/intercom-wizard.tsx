...
export function IntercomWizard({ open, onClose, onConnected }: IntercomWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // Intercom access tokens are JWT-ish (base64url segments separated
    // by ".") — quick sanity check rather than calling the API early.
    if (!token || token.length < 20) {
      setError(
        "Intercom Access Token is required. Find it in Intercom → Settings → Developer Hub → Your apps → [app] → Authentication."
      );
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/intercom/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, accessToken: token }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Intercom" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="Access Token"
          hint="Intercom → Settings → Developer Hub → Your apps → [app] → Authentication. Scopes must allow Read access on Contacts, Conversations, Tags, Segments."
        >
          <input
            type="password"
            required
