...
interface ActivecampaignWizardProps {
  open: boolean;
  onClose: () => void;
  onConnected: () => void;
}
...
export function ActivecampaignWizard({ open, onClose, onConnected }: ActivecampaignWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!/\.api-[a-z0-9]+\.com$/i.test(apiUrl)) {
      setError("API URL looks like https://<account>.api-us1.com — find it under Settings → Developer.");
      return;
    }

    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/activecampaign/connect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, apiUrl, apiToken: form.apiToken.trim() }),
      });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect ActiveCampaign" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="API URL"
          hint="ActiveCampaign → Settings → Developer → API Access. Copy the URL (https://<account>.api-us1.com)."
        >
          <input
            type="url"
            required
        ...
        <Field
          label="API token"
          hint="Same Settings → Developer page, the Key field below the URL."
        >
          <input
            type="password"
            required
