...
export function OracleWizard({ open, onClose, onConnected }: OracleWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/oracle/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Oracle" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field label="Service name" hint="The Oracle service name (or SID) for the database.">
          <input
            type="text"
            required
        ...
        <Field
          ...
          hint="Leave blank to replicate all user schemas. System schemas (SYS/SYSTEM) are always skipped."
        >
          <input
            type="text"
            value={form.schemas}
