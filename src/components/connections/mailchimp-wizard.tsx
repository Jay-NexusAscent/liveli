...
export function MailchimpWizard({ open, onClose, onConnected }: MailchimpWizardProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // Mailchimp keys are <hex>-us<NN> — the suffix selects the data center.
    if (!/-us\d+$/i.test(key)) {
      setError(
        "Mailchimp API keys must end with the data-center suffix (e.g. -us21). Copy the whole key from your account profile."
      );
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/connections/mailchimp/connect", {
        method: "POST",
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect Mailchimp" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="API key"
          hint="Find this in Mailchimp → Profile → Extras → API keys. The whole string including the -usNN suffix."
        >
          <input
            type="password"
            required
