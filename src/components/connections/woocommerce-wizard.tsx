...
export function WoocommerceWizard({ open, onClose, onConnected }: WoocommerceWizardProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Connect WooCommerce" maxWidth="max-w-lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <Field
          label="Store URL"
          hint="The storefront base URL, e.g. https://store.example.com"
        >
          <input
            type="url"
            required
        ...
        <Field
          label="Consumer key"
          hint="WooCommerce → Settings → Advanced → REST API → Add key (Read access is enough)."
        >
          <input
            type="password"
            required
