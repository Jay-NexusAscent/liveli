#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-linear target-bigquery`.
...
: "${TAP_LINEAR_AUTH_TOKEN:?Linear API key required}"
...
echo "→ source: linear → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-linear target-bigquery --state-id "$STATE_ID"
