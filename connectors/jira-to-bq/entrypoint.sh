#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-jira target-bigquery`.
...
: "${TAP_JIRA_EMAIL:?Atlassian account email required}"
: "${TAP_JIRA_API_TOKEN:?Atlassian API token required}"
: "${TAP_JIRA_DOMAIN:?Jira domain required (e.g. yourcompany.atlassian.net)}"
...
echo "→ source: jira ($TAP_JIRA_DOMAIN) → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-jira target-bigquery --state-id "$STATE_ID"
