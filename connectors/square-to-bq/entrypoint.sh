#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-square target-bigquery`.
...
: "${TAP_SQUARE_ACCESS_TOKEN:?Square access token required}"
...
echo "→ source: square → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-square target-bigquery --state-id "$STATE_ID"
