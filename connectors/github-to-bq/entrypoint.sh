#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-github target-bigquery`.
...
: "${TAP_GITHUB_AUTH_TOKEN:?GitHub Personal Access Token required}"
: "${TAP_GITHUB_REPOSITORIES:?At least one repository required (JSON array of \"owner/repo\")}"
...
echo "→ source: github (repos: $TAP_GITHUB_REPOSITORIES) → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-github target-bigquery --state-id "$STATE_ID"
