#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-activecampaign target-bigquery`.
...
: "${TAP_ACTIVECAMPAIGN_API_TOKEN:?ActiveCampaign API token required}"
: "${TAP_ACTIVECAMPAIGN_API_URL:?ActiveCampaign API URL required (https://<account>.api-us1.com)}"
...
echo "→ source: $TAP_ACTIVECAMPAIGN_API_URL (activecampaign) → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-activecampaign target-bigquery --state-id "$STATE_ID"
