#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-freshdesk target-bigquery`.
...
: "${TAP_FRESHDESK_API_KEY:?Freshdesk API key required}"
: "${TAP_FRESHDESK_DOMAIN:?Freshdesk domain slug required (the <co> in <co>.freshdesk.com)}"
...
echo "→ source: $TAP_FRESHDESK_DOMAIN (freshdesk) → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-freshdesk target-bigquery --state-id "$STATE_ID"
