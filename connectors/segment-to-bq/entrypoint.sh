#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-segment target-bigquery`.
...
: "${TAP_SEGMENT_API_TOKEN:?Segment api_token required}"
...
echo "→ source: segment → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-segment target-bigquery --state-id "$STATE_ID"
