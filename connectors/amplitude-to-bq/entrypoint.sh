#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-amplitude target-bigquery`.
...
: "${TAP_AMPLITUDE_API_KEY:?Amplitude API Key required}"
: "${TAP_AMPLITUDE_API_SECRET:?Amplitude Secret Key required}"
...
echo "→ source: amplitude → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-amplitude target-bigquery --state-id "$STATE_ID"
