...
: "${TAP_MSSQL_HOST:?mssql host required}"
: "${TAP_MSSQL_USER:?mssql user required}"
: "${TAP_MSSQL_PASSWORD:?mssql password required}"
: "${TAP_MSSQL_DATABASE:?mssql database required}"
...
echo "→ source: $TAP_MSSQL_HOST/$TAP_MSSQL_DATABASE → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
# with INCREMENTAL-mode streams wiped two years of demo data in
# LIVELI-111. The mssql tap defaults to FULL_TABLE which is safe under
# overwrite; keep it that way until the loader contract is fixed and
# PK gating equivalent to lib/postgres-introspection.ts has been built
# for SQL Server tables.
