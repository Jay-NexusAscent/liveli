#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-mixpanel target-bigquery`.
...
: "${TAP_MIXPANEL_API_SECRET:?Mixpanel Project API Secret required}"
...
echo "→ source: mixpanel → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-mixpanel target-bigquery --state-id "$STATE_ID"
