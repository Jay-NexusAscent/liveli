#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-closeio target-bigquery`.
...
: "${TAP_CLOSEIO_API_KEY:?Close API key required}"
...
echo "→ source: close → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-closeio target-bigquery --state-id "$STATE_ID"
