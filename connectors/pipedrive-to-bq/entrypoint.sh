#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-pipedrive target-bigquery`.
...
: "${TAP_PIPEDRIVE_API_TOKEN:?Pipedrive API token required}"
...
echo "→ source: pipedrive → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-pipedrive target-bigquery --state-id "$STATE_ID"
