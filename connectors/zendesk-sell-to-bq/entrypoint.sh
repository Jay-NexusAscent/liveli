#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-zendesk-sell target-bigquery`.
...
: "${TAP_ZENDESK_SELL_ACCESS_TOKEN:?Zendesk Sell access_token required}"
: "${TAP_ZENDESK_SELL_DEVICE_UUID:?Zendesk Sell device_uuid required}"
...
echo "→ source: zendesk-sell → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-zendesk-sell target-bigquery --state-id "$STATE_ID"
