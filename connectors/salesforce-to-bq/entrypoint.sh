...
# LIVELI-111). tap-salesforce IS on upsert and emits key_properties
# from `Id`; re-enabling would be safe — but keep it absent until
# there's a concrete need.

echo "→ state backend: $MELTANO_STATE_BACKEND_URI"
echo "→ state id: $STATE_ID"

meltano elt tap-salesforce target-bigquery --state-id "$STATE_ID"
