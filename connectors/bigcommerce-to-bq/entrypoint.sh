#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-bigcommerce target-bigquery`.
...
: "${TAP_BIGCOMMERCE_CLIENT_ID:?BigCommerce client id required}"
: "${TAP_BIGCOMMERCE_ACCESS_TOKEN:?BigCommerce access token required}"
: "${TAP_BIGCOMMERCE_STORE_HASH:?BigCommerce store hash required}"
...
echo "→ source: store $TAP_BIGCOMMERCE_STORE_HASH (bigcommerce) → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-bigcommerce target-bigquery --state-id "$STATE_ID"
