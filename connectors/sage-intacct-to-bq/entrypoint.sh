#!/usr/bin/env bash
# Reads connector config from env, runs `meltano elt tap-intacct target-bigquery`.
...
: "${TAP_INTACCT_COMPANY_ID:?Intacct company_id required}"
: "${TAP_INTACCT_SENDER_ID:?Intacct sender_id required}"
: "${TAP_INTACCT_SENDER_PASSWORD:?Intacct sender_password required}"
: "${TAP_INTACCT_USER_ID:?Intacct user_id required}"
: "${TAP_INTACCT_USER_PASSWORD:?Intacct user_password required}"
...
echo "→ source: sage-intacct (company $TAP_INTACCT_COMPANY_ID) → bq:$TARGET_BIGQUERY_PROJECT.$TARGET_BIGQUERY_DATASET"
...
meltano elt tap-intacct target-bigquery --state-id "$STATE_ID"
